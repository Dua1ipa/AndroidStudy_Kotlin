...
class ChatListAdapter : ListAdapter<ChatRoomItem, ChatListAdapter.ViewHolder>(differ) {
    ...
    companion object {  //Java의 static 멤버와 유사
        val differ = object : DiffUtil.ItemCallback<ChatRoomItem>() {  //리스트의 변경 사항을 감지할 때 사용하는 콜백
            override fun areItemsTheSame(oldItem: ChatRoomItem, newItem: ChatRoomItem): Boolean {  //두 UserItem이 같은 항목인지 확인 (userID로 비교)
                return oldItem.chatRoomID == newItem.chatRoomID
            }

            override fun areContentsTheSame(oldItem: ChatRoomItem, newItem: ChatRoomItem): Boolean {  //userID가 같더라도 내용이 변경되었는지 확인
